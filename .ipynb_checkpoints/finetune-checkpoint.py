'''
Methods to finetune pretrained models. Envisioned usage: finetune foundation model on specific dataset, prior to doing knowledge distillation to distill the finetuned foundation model into a smaller lightweight model.
Currently assuming dataset with single image patches as X, regression targets as y (ex: ST from patches).
TODOs: Support partial training, ie: instead of finetuning whole model, just finetune the last N layers.
'''
...
def train(model, train_loader, val_loader, loss_fn, optim, epochs, device, log_path, save_dir):
    ...
    for epoch in range(epochs):
        ...
        val_loss,val_true,val_pred = eval(model, val_loader, loss_fn, device)
...
def finetune_HEST_data(patches_path, adata_path, train_samples, val_samples, gene_list_path, log_dir, model, transforms):
    ...
    train(model, train_loader, val_loader, loss_fn, optim, epochs,
